def call(branchName = env.GIT_BRANCH, body) {
    ...
    if(branchName && (releaseStreams.collect { branchName =~ it ? true : false }).contains(true)) {
        println "[edgeXPRStage] Current branch IS a valid release branch, skipping code..."
    } else {
        println "[edgeXPRStage] Not a release branch, must be in a feature branch or PR"
        body()
    }
}