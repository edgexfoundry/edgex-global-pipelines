...
public class EdgeXReleaseOpenApiSpec extends JenkinsPipelineSpecification {
    ...
    def "Test call [Should] execute expected [When] openapi folder DOES exists" () {
        setup:
            getPipelineMock('fileExists').call(_) >> true
        when:
            edgeXReleaseOpenApi(validReleaseYaml)
        then:
            1 * getPipelineMock('echo').call('[edgeXReleaseOpenApi] Detected openapi folder. Validating release yaml.')
    ...
    def "Test call [Should] skip execution [When] openapi folder DOES NOT exists" () {
        setup:
    ...
    def "Test publishOpenApiChanges [Should] execute expected [When] called" () {
        ...
        then:
            ...
            1 * getPipelineMock('sh').call("sed -E -i 's|  version: (.*)|  version: 2.3.0|g' openapi/*.yaml")
